...
- `VÉRIFIÉ_CODE` : boucle d'agrégation sans garde d'entrée — `src/InvoiceCalculator.php:19-22`.
- `VÉRIFIÉ_CODE` : API Monolog 1.x (`addInfo`/`addError`) sans chemin de migration documenté — `src/AppLogger.php:23,28`.
- `VÉRIFIÉ_CODE` : constructeur `AppLogger` sans `try/catch` sur `StreamHandler` — `src/AppLogger.php:18`.

## Zones critiques

- **`src/InvoiceCalculator.php:19-22`** — la boucle d'agrégation est le cœur métier du projet. Un senior la regarderait en premier avant d'ajouter une règle (remise, avoir, taux par ligne) car elle ne tolère aucune données malformées sans erreur silencieuse.
...
- `HYPOTHÈSE` : **mode d'arrondi non documenté** — `round()` sans mode explicite pour des montants en CFP ; si la règle fiscale diffère de `PHP_ROUND_HALF_UP`, le résultat est incorrect pour certaines valeurs limites. Fichier : `src/InvoiceCalculator.php:30`.

## Recommandations priorisées

1. **Ajouter des gardes d'entrée dans `totalHorsTaxe`** — `isset($ligne['quantite'], $ligne['prixUnitaire'])` avec `throw new \InvalidArgumentException(...)` explicite. Fichier : `src/InvoiceCalculator.php:19-22`.
...
3. **Documenter le mode d'arrondi** — soit commenter l'appel `round()` avec la référence réglementaire CFP, soit passer `PHP_ROUND_HALF_UP` explicitement pour signaler l'intention. Fichier : `src/InvoiceCalculator.php:30`.
